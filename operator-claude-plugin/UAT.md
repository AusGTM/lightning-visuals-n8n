...
| 2.6 | Give it something unreadable — an empty file, a PDF of a photo | A **clear, actionable** error. Never a silent drop, never zero rows with no explanation | **FIXED in 0.7.1, awaiting operator walk** — the `.pdf` half always passed (`UnsupportedFileError`). The empty-file half failed (0 rows, no explanation, no skill branch) and is fixed: the skill now stops at the preview, names the likely causes, and offers no arming phrase. Pinned two-sided, red-checked. No operator has walked the new behaviour |
