...
| 2.2 | Give it a CSV or XLSX with messy headers (`E-mail Address`, `Ph.`) | Reads them without you renaming anything first | **FAIL against the criterion as written** (operator walk 2026-08-04, 0.7.3 — the first build that could show header mapping at all). Neither named alias exists: the table has `email address`/`e-mail` but not `e-mail address`, and `phone`/`mobile`/`tel` but not `ph.`. 6 of 7 headers drop. The plugin predicted it correctly and refused to present the file as send-ready — the requirement and the mapping disagree, not the code. The earlier PASS was real but tested `Email Address`/`Phone`, which do map. Todo `2026-08-04-uat-22-names-aliases-the-mapping-lacks` |
