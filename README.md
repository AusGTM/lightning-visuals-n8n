...
| n8n Cloud workflows (contact ingest, enrichment, scheduled maintenance, backend status, review decision, suggest discovery) | ✅ **6 deployed on n8n Cloud, all six active** (node counts as of Phase 75, 2026-09-20: backend status 33, contact ingest 101, enrichment 289, review decision 55, scheduled maintenance 43, suggest discovery 26 — CLAUDE.md §13.0.2 / §10.3.3), write gates disarmed at rest |
...
| Committed `n8n/*.json` vs the running n8n Cloud instance | ✅ **level as of 2026-09-20 (Phase 75 plan 06)** — enrichment, scheduled maintenance, contact ingest and review decision redeployed disarmed and bounced (289/43/101/55 live = committed); backend status 33 and suggest discovery 26 unchanged since the 2026-09-18 Phase 73.1 deploy. `settings.executionOrder: "v1"` on all six, all four `ALLOW_HUBSPOT_*` flags `false` (CLAUDE.md §10.3.3). Nothing armed. Do not read an in-repo node as proof of what n8n is running — only a live read is |
| Config-driven region whitelist + scoring-version staleness (`regions.home` 12 codes, veto reason `Outside target regions`, `lv_icp_scoring_version` live, fourth kill switch `ALLOW_HUBSPOT_RECOMPUTE_WRITES`, version-stale recompute reroute, SJ-2 backstop) | ✅ **Phase 75, complete 2026-09-20** — verification 20/20 decisions; two disarmed recompute proofs (`12682`, `12683`); geography flow `4626722240` revision 14. **Deliberately not done:** the D-75-17 flag flip and the first supervised bump sweep (`docs/OPERATOR-RESCORE.md`); five records still carry the legacy `Non-ANZ geography` string until that sweep runs. Backend `0.22.0` |
