...
### Added
- **Provenance-aware `manual_protected` (quick 260904-pav).** The class no longer refuses
  without asking who wrote the value it is protecting. A canonical value may now be
  corrected by the pipeline's own later, better answer when — and only when — **all four**
  of these hold, each of them fail-closed: the existing `lv_enrichment_provenance` entry's
  `source` is listed in the field's new `system_correctable_sources` allowlist; the entry's
  recorded `value` still **equals** the current value (so a human retype since the system
  wrote it refuses); the row carries no material provider conflict (`opts.rowConflicted ===
  false` strictly, reusing the size-conflict detector that *is* the `harveynorman.com.au`
  franchisor/parent-domain guard, so a caller that omits the flag fails closed); and the
  field's own existing `min_confidence` passes. `companies.domain`'s allowlist is
  `["create_seed"]` alone — every other source's `domain` entry is a *staged refusal*, and
  admitting one would let a refused candidate authorise its own promotion. Landed in
  `n8n/code/mergeCompanies.js` and `src/merge_policy.py` in one commit per the Phase 46
  parity rule, with `n8n/*.json` regenerated by `scripts/build_cloud_workflows.py`.
- **The company-create branch now stamps provenance for the domain it seeds.** BUG 19's
  create seed wrote `properties.domain` with no provenance entry at all, so the correction
  path above had nothing to key on. It now records `source: "create_seed"`,
  `validation_status: "request_echo"` (a new status, added to CLAUDE.md §6.1) and
  `confidence: 0` — an identity echo of the request must never read as evidence.
- **Search fallback for suggest-contacts (quick 260904-5sd, client-side).** New
  `operator-claude-plugin/scripts/search_fallback.py` and a committed
  `config/source_allowlist.yaml`. When the sitemap ladder finds nobody, the client may fall
  back to web search — but only after a ladder that ended **cleanly**: any `refused`
  attempt anywhere blocks the path order-free, while `empty` and `cap_exhausted` open it (a
  budget we imposed on ourselves is not a fence the site put up). An absent, unknown or
  unreadable disposition, an empty list, and a non-list are all ineligible **without
  raising**. Ranking reads the **URL host alone** — never a snippet — so an unverifiable
  transcription can never become a row field: rank 1 the company's own host, rank 2
  LinkedIn, rank 3 a curated `[ASSUMED]` industry allowlist, and a host on no rank is
  **rejected** rather than ranked last. `hold_weak_sources` is a second, records-level gate
  independent of the waterfall: a rank-3 row is always held with its source URL quoted,
  however confidently validated.

### Changed
- **`lv_enrichment_provenance` is now written additively by the enrichment lane.** It
  previously replaced the whole blob (the additive guarantee was the review lane's alone),
  which would have destroyed the create seed on the first enrich run and was already losing
  every untouched field's entry.
- **A companies row reports a companies-shaped match reason (quick 260904-5a8).**
  `summarizeMatch` gained a `lane === "company"` arm reached by a call-site literal in
  `ENRICH_DECIDE_CO_CLOUD`; a company row no longer reads the contacts sentence "no
  searchable identity — the row has no email, object id, or name+company pair". The arm
  deliberately keeps `tier: "unknown"` / `auto: false` and never reads `existingRecord` —
  `match.tier` **gates** downstream, so a `"high"` verdict would have flipped a hold into a
  send. This is reporting, never gating. `laneOf` is untouched.

### Fixed
- **A double-encoded job title no longer silently misses its role family (quick 260904-447).**
  The portal stores double-encoded entities, and a single `html.unescape` pass leaves an
  orphaned `amp` token wedged mid-run. Fixed at **both** seams with a bounded fixed-point
  loop — including the shipped match-time path, where the bug dropped a contact rather than
  merely writing an ugly literal.

### Known limitations
- **The provenance-aware correction is inert.** Nothing in the pipeline proposes a company
  `domain`: no `normalizeProviders.js` branch supplies one, and the company providers are
  looked up *by* the record's domain, so they structurally cannot contradict it. The
  mechanism is correct and tested; no record is corrected yet. Filed as
  `.planning/todos/pending/2026-09-04-company-domain-has-no-candidate-source.md`.
- **The committed `n8n/*.json` is AHEAD of the running n8n Cloud instance.** Phase 62 and
  both n8n-touching quick tasks regenerated and committed the workflow JSON **without
  deploying** (CLAUDE.md §13.0.2). Do not read an in-repo node as evidence of what n8n is
  running. Nothing is armed, and the first live unattended, credit-spending batch has still
  not run.
