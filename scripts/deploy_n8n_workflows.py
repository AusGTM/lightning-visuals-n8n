...
NODE_CREDENTIAL_MAP = {
    ...
    # Phase 27 Plan 01 — the four HubSpot count searches added to the status endpoint's
    # chain (requested-unresolved / awaiting-review, companies + contacts). An unmapped
    # HubSpot node deploys UNBOUND and 401s only at activation, the failure class this
    # repo has already been bitten by three times.
    "HS Requested Search (Companies)": {"cred_type": "hubspotAppToken", "cred_name": "LV HubSpot"},
    "HS Review Search (Companies)": {"cred_type": "hubspotAppToken", "cred_name": "LV HubSpot"},
    "HS Requested Search (Contacts)": {"cred_type": "hubspotAppToken", "cred_name": "LV HubSpot"},
    "HS Review Search (Contacts)": {"cred_type": "hubspotAppToken", "cred_name": "LV HubSpot"},
    ...
    # Phase 73.1 Plan 07 (D-06) — the sixth cloud workflow's trigger and its one provider
    # search node. The trigger binds the SAME shared webhook-secret credential every
    # other trigger in this file binds — one operator secret, six endpoints. The
    # ZoomInfo search node reuses the SAME credential the enrichment lane's ZoomInfo
    # hops already bind (D-06: "reuses existing n8n credentials", no new registration).
    # Plan 09 (D-12, operator ruling 2026-09-18): Apollo and Lusha search nodes were
    # dropped from this lane entirely (search-only, ZoomInfo retained) -- their
    # credential entries are removed here, not left bound to nothing.
