...
there is nothing left to opt into. ~~Three request-level flags remain.~~ **Corrected 2026-09-13:**
**TWO** remain — `recompute` and `source_by_field`. The second of the two Phase 61 flags,
`scale_up`, was also retired (D-70-24, 2026-09-10, executions `12211`–`12348`), which §13.0.2's
own heading and opening paragraph already record; this sentence was never updated for it.
Verified against the committed `n8n/wf_enrichment_cloud.json`, which carries zero
`n8n-nodes-base.executeWorkflow` dispatch nodes (its single `executeWorkflow*` hit is the
`executeWorkflowTrigger` entry point, a different node type), and
`tests/n8n/scaleUpRefused.test.mjs`. See §13.0.2.
...
**Corrected 2026-09-13:** the counts in that sentence were stale in both directions (it said
"four signals", then named three, then generalized over "all three"). **No scheduled path
carries either of the two remaining signals.** SJ-3 and every other schedule trigger POST no
`recompute` and no `source_by_field`, so §19.1's statement stands unchanged and generalizes:
**both** are on-demand only. `async_ack` and `scale_up` are retired per this section's own
D-70-07 and D-70-24 notes above, so no scheduled path can carry them either. Verified against
`n8n/wf_scheduled_maintenance_cloud.json`'s `SJ-3 Build Dispatch Event` node, whose jsCode emits
only `objectId` / `objectType` / `subscriptionType` / `occurredAt` — no flag key of any kind.
...
> **Superseded 2026-09-10 (Gates 10/11/12 — phase complete, commit `ec102a4`).** Gate 10
> deployed and bounced the committed v1 bundle (287/69/55/43/30 nodes, `settings.executionOrder:
> "v1"` read back on all five); Gate 11 re-ran the disarmed D-70-19 proof on it (executions
> `12354`-`12358`, `execution_order_all_v1: true`, `shapes_equal` on all four sends, 0
> connection violations, none of Gate 8's symptoms); Gate 12 ran the first ARMED write on the
> v1 graph (execution `12363`, one contact, window opened and closed in-session). **Live state
...
| **`settings.executionOrder` is ABSENT on all five running cloud workflow bodies** (the engine's own legacy-v0 default, not a value anyone set). Read directly from the LIVE workflow bodies via the executions API, not from the committed JSON (which also omits it — see D-70-02). | `[observed live]` (2026-09-10, `LV Enrichment (Cloud template)` and `LV Contact Ingest (Cloud template)` both `null`, `70-RUNTIME-VERDICT.json`) — **historical as of Gate 10, 2026-09-10 (D-70-28):** `"v1"` has since been read back on all five live bodies (Gate 10 deploy, Gate 11 driver pre-send read, Gate 12 post-disarm read) |
| **Legacy execution order (`executionOrder` absent or not `"v1"`) pushes ONE empty item (`{ json: {} }`) onto every node on an empty branch via `addNodeToBeExecuted`'s `addEmptyItem` branch, so a waiting multi-input node can still execute** — the mechanism behind every Gate 8 symptom and G-70-2/G-70-3/G-70-5: a gate Code node fed the empty item runs and stamps its marker, an HTTP node fed it sends a request with an empty id, an `Execute Workflow` node fed it dispatches. | `[documented]` (`packages/core/src/execution-engine/workflow-execute.ts`, `addNodeToBeExecuted`, the `addEmptyItem` branch — n8n's own execution-order docs page describes only branch ordering and says nothing about empty-input execution at all) + `[observed live]` by contrast: the push is present on every legacy-order execution (`12349`-`12353`, next-but-one row) and absent on every v1 execution of the same graphs (`12354`-`12358`, the row below) — the executionOrder flip was the ONLY variable between the two runs |
| **Under v1 there is no such push: a node on an empty branch does not run, and at end-of-run every node still `waitingExecution` executes once with whatever inputs arrived, gated on `requiredInputs`** — `[0, 1]` for `chooseBranch` mode, `1` for every other mode. Every Merge this repo generates is typeVersion 3.2 in `append` or `combine` mode, so `requiredInputs` is always `1` here: enrichment 10 `append` + 23 `combine`, ~~ingest 2 + 8~~, review 3 + 7, backend_status 7 `combine`, maintenance none. (**Corrected 2026-09-13:** ingest is **3 `append` + 9 `combine`** — Phase 72 widened `wf_contact_ingest_cloud.json` from 69 to 78 nodes; counted from the committed JSON, which also confirms the other four figures unchanged. The row's load-bearing claim is untouched: every Merge is still typeVersion 3.2 in `append`/`combine`, so `requiredInputs` is still `1`.) | `[documented]` (`packages/nodes-base/nodes/Merge/v3/actions/versionDescription.ts`, `requiredInputs`) + **`[observed live]`** (Gate 11, 2026-09-10, executions `12354`/`12355`/`12356` enrichment and `12357`/`12358` ingest, `70-RUNTIME-VERDICT.json` `execution_order_all_v1: true`, frozen at `tests/n8n/fixtures/frozen/exec_1235{4..8}.runData.json`: `HubSpot Update`, `IF List Expanded` and `Apply Contact Judge Verdict` never ran; the live lane's own D-70-23 sentinel gate did not run; `Enrichment Gate Merge` fired exactly once with 6 items = 4 absent-lane markers + 2 real rows; `Build Response` emitted every real row; 0 runData-source-vs-declared-connections violations. Armed form: Gate 12, execution `12363`, `70-UAT.md` Test 12 — one contact updated + associated, the other row `write_blocked`, nothing else touched. The end-of-run drain itself was observed too — see the "fire twice" row below) |
...
| **A Merge fires at most once per execution — LEGACY execution order only.** Once every declared input has received its first delivery (empty or not), the Merge runs and locks; nothing arriving afterward re-triggers it. **Does NOT hold under v1** — see the "can fire twice" row below. | `[observed live]` (`12203`, `12206` — both Merges ran exactly once, `settings.executionOrder` absent on both bodies) |
| **Under v1 a Merge CAN fire twice in one execution.** `Decide Company Action Merge` (append, 2 declared inputs, SIX producer edges) ran twice on each of `12354`/`12355`/`12356`: run 0 with 2 marker items, both inputs sourced from `Companies Absent Sentinel Gate`; run 1 with 1 marker item, input 0 sourced from `Recompute Not Requested Sentinel Gate` and input 1 `null` — the documented v1 end-of-run drain (`requiredInputs: 1`) firing on a single arrived input. `Decide Company Action` therefore ran TWICE, 0 items each (it filters markers), and nothing downstream ran. Harmless here only because the consumer tolerates a marker-only run; a consumer doing real per-run work behind a Merge with more than one producer per input would double-run. The walker has modelled the end-of-run drain since quick task 260911-0tz, and as of quick task 260911-1z5 reports this exact shape — a Merge that FIRED with an unfilled input — as a `merge_fired_with_unfilled_input` entry in `trace.stalled`, pinned live by `tests/n8n/walkerEngineFidelityV1.test.mjs` against `12354`/`12355`/`12356`. | `[observed live]` (2026-09-10, `12354`/`12355`/`12356`, pinned by `tests/n8n/v1RuntimeRecordings.test.mjs`) |
| **Under v1 a node that RAN and emitted zero items is NOT a delivery to a Merge input.** `Merge Company` ran with 0 items into `Decide Company Action Merge` input 1 on `12354`/`12355`/`12356` and never appears in that Merge's `source` on either run. This is the question `walkWorkflow.mjs` D-70-30 rule (c) recorded as UNOBSERVED: the walker's rule (c) (zero-item output still delivers, from legacy `12203`/`12206`) models the legacy engine only — the v1 flip (a zero-item output makes NO delivery at all) is modelled as its own explicit branch in `propagate()`, pinned live by `tests/n8n/walkerEngineFidelityV1.test.mjs` against `12354`/`12355`/`12356`. | `[observed live]` (2026-09-10, `12354`/`12355`/`12356`, pinned by `tests/n8n/v1RuntimeRecordings.test.mjs`) |
...
| **`alwaysOutputData` rescues output index 0 only — never whichever output happens to be empty.** D-74-03 (Phase 74, CR-01): the substitution guards on `nodeSuccessData?.[0]?.[0]` and writes only `nodeSuccessData[0]`; a later output (index 1, an IF's false branch or an `onError: "continueErrorOutput"` node's error branch) is never rescued, even when THAT is the branch that's empty. `tests/n8n/lib/walkWorkflow.mjs`'s `propagate()` used to pad whichever output was empty, ignoring `outputIndex` entirely — every offline graph proof this repo has rested on that wrong rule until this fix. | `[documented]` (`packages/core/src/execution-engine/workflow-execute.ts`, `ensureAlwaysOutputData` — file+symbol only, this repo does not vendor that source) + `[observed live]` (execution `12522`: `HubSpot Create` outs `[21, 0]` — output 1, its error branch, stays empty and delivers nothing to `Create Carry Merge` input 2; pinned by `tests/n8n/walkerEngineFidelityV1.test.mjs`) |
| **A zero-rejection create batch completes via the v1 end-of-run drain in ONE pass, nothing lost, when the drain fires the most-upstream qualifying Merge first.** `Create Carry Merge` (no Merge upstream of it) drained before `Ingest Merge Response` and `Build Association Request Merge` (both reachable FROM it, several hops downstream) — 42 items, then 22, then all 46 rows reached `Build Ingest Response` once each, no `merge_pending_runs_undrained`. The walker's own drain scan used to pick a plain `Object.entries(mergeState)` insertion order, which could (and on this exact graph, did) pick a downstream Merge first, using up its one-drain cap (MN-01) before the real delivery arrived — reported as a false loss; the scan now prefers a qualifying Merge no OTHER qualifying, not-yet-drained Merge can reach via `connections` (D-74-03). The enrichment lane's own research-error branch (D-74-14, `IF Research Errored` output 1) is the SAME index-0-only padding gap in a different node — fixed graph-side with a starved-lane sentinel — but stays `[documented]` only for a *live* observation: D-74-11's disarmed proof send does not exercise it. | `[observed live]` (execution `12522`, `Create Carry Merge` 1×42, `Build Association Request Merge` 1×22, `HubSpot Associate Company` 1×22, `Ingest Merge Response` 1 run / 6 inputs, `Build Ingest Response` 1×46; the DRAIN ORDER itself is also observed, not just the counts — `runData` carries unredacted `executionIndex`/`startTime` per node run: `Create Carry Merge` executionIndex 59 (startTime 1789643226047) < `Build Association Request Merge` 61 (1789643226072) < `Ingest Merge Response` 66 (1789643227943) < `Build Ingest Response` 67 (1789643227945) — strictly increasing in the most-upstream-first order the fix predicts; pinned by `tests/n8n/walkerEngineFidelityV1.test.mjs`) |
...
**Gate 11 ran 2026-09-10 (executions `12354`-`12358`) and Gate 12 followed (`12363`); the
upgrade above is done.** The v1 `requiredInputs` row now carries `[observed live]`; the legacy
`addEmptyItem` row carries the by-contrast observation; the "ABSENT on all five" row is marked
historical; the legacy "fires at most once" row is scoped to legacy; and two NEW v1 rows record
what Gate 11 showed beyond its pass criteria — a Merge CAN fire twice under v1 (the end-of-run
drain on a single arrived input), and a zero-item OUTPUT is NOT a delivery under v1. Both are
pinned by `tests/n8n/v1RuntimeRecordings.test.mjs` over the five frozen recordings. The walker
now models BOTH engines as two explicit branches keyed on `settings.executionOrder`, with
