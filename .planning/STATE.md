...
last_updated: "2026-09-20T11:29:19.330Z"
last_activity: 2026-09-20
last_activity_desc: Phase 75 complete
state_head: 4bc2c8ffc7c6846d6ba976113fdac479e99765d9
progress:
  total_phases: 13
  completed_phases: 4
  total_plans: 79
...
Last activity: 2026-09-20 — Phase 75 complete
...
**Last session:** 2026-09-20T07:24:12.776Z
...
| Phase 72 P04 | 240min | 3 tasks | 25 files |
...
| Phase 73 P01 | 55min | 3 tasks | 12 files |
| Phase 73 P02 | 20min | 2 tasks | 7 files |
| Phase 73 P03 | 55min | 3 tasks | 11 files |
| Phase 73 P04 | 45min | 2 tasks | 4 files |
...
| Phase 75 P03 | 33min | 3 tasks | 18 files |
| Phase 75 P04 | 42min | 3 tasks | 10 files |
...
- [Phase 71]: 71-03 Task 3 (D-71-06 live gate) ran by the operator 2026-09-12, PASS on both surfaces -- Jimmy Busteed read new_person on the batch surface (create all 2, landed associated to 9605284724), Louise White read new_person and was created from a fresh review-triage cold start, neither re-offered on a third open, Grant Dewsbury persisted. Findings F71-1 (shared run_manifest positional pollution) and F71-5 (ingest lane drops paid-for enrichment extras) triaged to follow-up-phase todos (commit 341a9190). Clean-up (3 HubSpot contacts, held_queue.json, run_manifest.json, driver scripts) still owed to the operator, not performed by any executor session.
- [Phase 72]: D-72-22 (operator ruling): provider-sourced ingest fields carry provider-grade confidence (85) via a per-field confidenceByField map keyed off source_by_field, instead of the flat csv 80. — Discovered mid-task via the tracer's own dry run; ruling recorded in 72-CONTEXT.md D-72-22.
- [Phase 72]: hs_linkedin_url's producer-having-but-never-chased status generalized the fetch-gate
...
- [Phase 72]: D-72-06/07/09: recency/TTL gate lands in all three merge engines in one commit, per-field (not per-call) source resolution via opts.sourceByField
- [Phase 72]: D-72-08: system-correctable predicate generalizes unchanged to stale_refreshable fields (contacts.jobtitle, companies.industry); companies.domain's create_seed list untouched
- [Phase 72]: Task 3 scope held to the ingest lane only; enrichment-lane contacts branch and companies branch have no history hop (design gap filed, WINDOWS.md)
- [Phase 72]: D-72-23: create the three overflow-slot properties live in plan 05 (not plan 08), via sync_hubspot_properties.py's two-key gate, so the schema-coverage guard admits Task 2's engine references.
...
- [Phase 73.1]: D-11c closed: role_vocabulary.yaml v4 adds Executive Officer and Board Chair families, excluding Chairman/Chairperson (already in Chair) per the new duplicate-member guard. — Pickleball round 2026-09-18 dropped these two titles; classify_title's longest-wins tie-break must never depend on YAML order.
...
- [Phase 74]: WR-12: apply_dedupe's default output location changed to the input's own resolved parent directory (writes beside the input), not the review's hash-tagged-scratch-dir variant -- matches the plan's own <action>/<acceptance_criteria> literally; explicit scratch_dir override unchanged for all 7 pre-existing callers.
- [Phase 74]: WR-04: read_collapsed_block(path) raises for ANY read failure once --collapsed was explicitly passed (missing, unreadable, or malformed JSON) and returns None only when --collapsed was never passed at all -- the plan's 'simply absent' bullet is read as 'flag not given', matching 73-REVIEW.md's own fix snippet verbatim per D-74-10's binding directive to use the review's fixes.
...
- [Phase 75]: Phase 75 Plan 03: taught _write_gate_js to read write_request.action==='recompute', the missing wiring half that makes ALLOW_HUBSPOT_RECOMPUTE_WRITES reachable end to end (plan text assumed it already existed).
- [Phase 75]: Phase 75 Plan 03: version-stale reroute (ENRICH_CO_GATE/SJ2_CO_GATE) gates on !row.lookup_failed -- a transport-failure row has no real existingRecord to be stale about; found via full-suite fallout.
- [Phase 75]: Phase 75 Plan 03: version-stale skip rides the existing IF Company Recompute lane via a row marker, not a new IF spliced onto IF Company Skip -- zero new nodes, verified safe against append-mode Merge + marker-filter semantics.
- [Phase 75]: Verified HubSpot's PATCH /crm/v3/properties contract live via public docs before implementing the sync tool's option-update path — Assumption A1 flagged the shape as unverified; confirming method/path/replace-semantics up front avoided implementing a guessed contract
...
- tests/test_hubspot_schema_coverage.py is red: wf_enrichment_cloud.json's Decide Company Action references lv_icp_scoring_version, which the live HubSpot portal does not yet hold. Deliberate per D-75-10 (property CREATE is Plan 05's job); tracked in .planning/WINDOWS.md (unmet-truth). Closes when Plan 05 runs sync_hubspot_properties.py live.

### Quick Tasks Completed

| # | Description | Date | Commit | Status | Directory |
|---|-------------|------|--------|--------|-----------|
| 260911-3mu | Close second-review findings on the walker v1 model: combine-mode annihilation now a reported loss, grouping rule scoped to what Gate 11 observed, detector pinned on the recordings, Merge-free cycle guard | 2026-09-11 | 5ca774a0 | Verified — four review passes; fourth: no blocker, no major; nits closed in d30cad69 | [260911-3mu-close-second-review-findings-on-the-walk](./quick/260911-3mu-close-second-review-findings-on-the-walk/) |
...
| 260905-rf1 | The `suggest-contacts` role filter no longer selects 0 on a club that uses one-word titles. Live 2026-09-04 (Brisbane Roar): the ladder found 3 real staff on the club's own contact page and the filter dropped ALL of them — `Marketing`, `Media` and `Sponsorship` each classified to `None`, and only an operator override rescued the round. **The cause was the MATCHER, not the vocabulary:** `classify_title` needs a family member's tokens contiguous, and no shipped member was the single token `marketing`. **Deriving is not the fix and was ruled out by measurement, not preference** — the derived vocabulary misses the same three titles, and adopting it is a recorded REJECTED decision (drops 14 curated families, every racing-governance one). The plan's other candidate — matching a single-token title against a family whose LABEL starts with that token — was killed the same way: measured, `marketing -> ['Marketing Manager']`, `media -> []`, `sponsorship -> []`, so it fixes 1 of 3 proven cases. Fix landed as **three new families appended at the END** of the curated file, one bare member each, `role_classify.py` untouched (zero diff). Append-at-end is load-bearing, not cosmetic: `classify_title` breaks an equal-length tie with a strict `>` , so YAML order wins, and every in-place home measurably FLIPPED `Marketing Director` / `Media Director` out of `Board & Committee`. Appending flips nothing — independently re-verified post-merge. Collision audit over the post-edit vocabulary: zero (no single-token member in two families). Four of the todo's seven suggested short forms (`communications`, `commercial`, `operations`, `finance`) were deliberately NOT added — no live evidence, and two would tie-flip. **Honest limit, stated not papered over:** `select_people` still gates on the operator's ticked `chosen_families`, so this makes a selection that catches those 3 people POSSIBLE where none could before; it does not retroactively make that round yield 3. Plugin 0.39.1 -> 0.39.2. Root 4206/154 (was 4189), plugin 2448/5 (was 2431), node 894/0 unchanged; `n8n/` zero diff. Zero live calls. | 2026-09-05 | 8da9bf1 | Complete (no verifier ran — quick task without `--validate`) | [260905-rf1-role-filter-one-word-titles](./quick/260905-rf1-role-filter-one-word-titles/) |
| 260905-ad2 | A company may now carry MORE THAN ONE known domain, so an organisation whose website and mail sit on different registrable domains stops having every genuine staff email held. Live case (Roma Turf Club, 2026-09-04): the recorded `romaturfclub.com.au` is **correct** — the round's own ladder fetched `www.romaturfclub.com.au/sitemap.xml` and read the committee page from that host — while the club's published address is `INFO@romaturfclub.org.au`, so every real committee email read as `email_domain_mismatch`. The record was INCOMPLETE, not incorrect, which is why this is NOT the Brisbane Lions wrong-domain bug and why "correcting" the domain would have broken the discovery ladder. **The match rule is untouched:** an `any()` over the byte-identical `ed == cd or ed.endswith("." + cd)` applied PER DOMAIN — never the tempting shared-registrable-label rule, which is unsafe because `<label>.com.au` and `<label>.org.au` are separately registrable in Australia and can be different organisations (exactly how `craig.smith@thehartford.com` was correctly held in that same round). `partition_for_dispatch`'s signature and body are unchanged and `company_domains` stays REQUIRED with no default — verified post-merge, it still raises `TypeError` on one argument, so the operator ruling keeps its enforcement point. **D-ad2-03: alternate domains are OPERATOR-SUPPLIED ONLY.** `mailto:` harvesting was rejected, not deferred for convenience: the crawl ladder is bound to the RECORDED host, and whether that host is right is precisely what the open Brisbane Lions todo says is unreliable — auto-adoption would compose two open bugs into a wrong record widening its own send gate with no human in the loop. Propose-and-confirm recorded as the deferred path; an operator naming the domain already IS the confirmation. Independently re-verified post-merge across 7 cases: Roma passes WITH the alternate and still HOLDS without it, `thehartford.com` still holds, `romaturfclub.com.au.attacker.tld` still rejected, a company with no entry still `company_domain_unknown`, freemail still `freemail`. Plugin 0.39.2 -> 0.40.0. Root 4216/154 (was 4206), plugin 2458/5 (was 2448), node 894/0 unchanged; `n8n/` zero diff. Zero live calls. | 2026-09-05 | 900db57 | Complete (no verifier ran — quick task without `--validate`) | [260905-ad2-company-alternate-domains](./quick/260905-ad2-company-alternate-domains/) |
| 260911-anv | Todo 2026-08-04-enrichment-throughput-ceiling (operator ruling: MEASURE FIRST, no gate change): (a) make Judge Gate emit its reasons[] on the row it passes downstream so a run's judge-trigger distribution is readable from runData, via scripts/build_cloud_workflows.py regeneration only (never hand-edit n8n/wf_*.json, no deploy, no arming); (b) confirm the web-research max_uses actually in effect in the committed n8n/wf_enrichment_cloud.json against WEB_RESEARCH_MAX_SEARCHES=5 and record the answer in the todo; (c) if existing n8n executions are readable via the API with zero new executions and zero writes, sample Judge Gate runData over a handful of past runs and record the reasons[] distribution in the todo. Read the todo's "Operator ruling 2026-09-11" section first. RO-2 test test_judge_spec.py::test_ro2_judge_gate_cannot_see_size_conflicts must stay green. Band bounds stay [75,85]. | 2026-09-11 | 253a75b5 | — | .planning/quick/260911-anv-todo-2026-08-04-enrichment-throughput-ceiling-operator-rulin |
| 260911-anw | Todo 2026-09-04-walk-provenance-locator-names-last-page-only (warning, 64-REVIEW WR-01): a synthesised row's provenance locator names the LAST page fetched, not the page the person was found on. Carry a per-person source_url through walk_pages' fold in operator-claude-plugin/scripts/suggest_contacts.py (the scores list already has per-page url) and have synthesise_rows / SKILL.md step 7 use it instead of pages[-1]["url"]. Keep walk_pages' six documented return keys; add source_url on each admitted person rather than a seventh top-level key. Test: two-page walk, person from page 1 gets page 1's URL. | 2026-09-11 | 901825c1 | — | .planning/quick/260911-anw-todo-2026-09-04-walk-provenance-locator-names-last-page-only |
| 260911-anx | Todo 2026-09-07-merge-enriched-ignores-jobtitles-own-protect-if-current-present (operator ruling: PER-FIELD, BOTH CALLERS): merge_enriched in operator-claude-plugin/scripts/preingest.py reads protect_if_current_present per field from the plugin's shipped config/field_policy.yaml for both enrich-before-ingest and suggest-contacts. jobtitle (false) accepts a differing waterfall value; email and location fields (true) are never replaced; conflicts list still records every replaced value. No per-field min_confidence in this task. Tests: operator-typed email never replaced; jobtitle DOES refresh; a field absent from policy keeps today's fill-only behaviour. Read the todo's "Operator ruling 2026-09-11" section first. | 2026-09-11 | d5d38588 | — | .planning/quick/260911-anx-todo-2026-09-07-merge-enriched-ignores-jobtitles-own-protect |
| 260911-ao0 | Todo 2026-09-09-written-records-labels-propose-and-enrich-legs-failed (major): written_records records propose/enrich legs, which write nothing, as "None -> failed" in the end-of-run report. Implement the todo's "## Fix shape" section in operator-claude-plugin/scripts/written_records.py so a leg that writes nothing is reported as such, never as failed. Test on the recorded run shape. | 2026-09-11 | a6c2e5fa | — | .planning/quick/260911-ao0-todo-2026-09-09-written-records-labels-propose-and-enrich-le |
| 260911-ao1 | Todo 2026-09-11-merge-input-contract-allows-many-producers-per-input (operator ruling: OPTION A, ALLOWLIST + ASSERT): assert_merge_input_contract in scripts/build_cloud_workflows.py gains rule 5: a Merge input with more than one producer edge is a build-time violation unless the Merge is named in an explicit tolerant-allowlist dict carrying a reason string; Decide Company Action Merge goes on it (consumer filters markers; only marker-only runs multi-fire). Collect Credits is admitted by the allowlist or by a shared-upstream-gate exclusivity check, implementer's choice, tested either way. Regenerate all wf_*.json via the builder and verify zero JSON diff (no graph change, no deploy). Mirror the rule in tests/n8n/mergeInputContract.test.mjs. RED first: the test must fail before the allowlist entry exists. Read the todo's "Operator ruling 2026-09-11" section first. MN-01 and NF-MJ-01 stay open. | 2026-09-11 | 20f5fbf3 | — | .planning/quick/260911-ao1-todo-2026-09-11-merge-input-contract-allows-many-producers-p |
| 260911-any | Todo 2026-09-08-forbidden-name-markers-refuse-secretary-and-armidale (major): inherited forbidden-name substring markers refuse real club data ("Secretary" trips "secret", "Armidale" trips "arm"). Read the todo's "Candidate fixes" section and implement the smallest one that keeps the guard's intent (word-boundary or whole-token match rather than substring). Tests: "Secretary" and "Armidale" pass; the original forbidden tokens still refuse. | 2026-09-11 | 1b19432c | — | .planning/quick/260911-any-todo-2026-09-08-forbidden-name-markers-refuse-secretary-and |
| 260911-anz | Todo 2026-09-09-enriched-preview-says-send-for-rows-the-confidence-gate-holds (major): the enriched preview labels a row SEND while confidence.assess holds it, two verdicts for one row. Implement the todo's "## Fix" section so the preview shows the ONE verdict the gate will actually apply. Test on the UAT Round B shape recorded in the todo (run 2bc3617b094b4c939d57f38ff6704e3f). | 2026-09-11 | 228745ef | — | .planning/quick/260911-anz-todo-2026-09-09-enriched-preview-says-send-for-rows-the-conf |
| 260911-ao2 | Todo 2026-09-04-website-less-company-search-fallback (operator ruling: SEARCH, LINKEDIN-OR-HELD): a company with no usable website may reach the web-search fallback from discovery_plan's empty-candidates branch in operator-claude-plugin/scripts/suggest_contacts.py. No own-host means no rank 1; rank 2 (linkedin.com) rows may be sendable, rank 3 always held; a search-discovered domain is never written back as website. MAX_FALLBACK_SEARCHES still bounds the round, no while loop (tests/test_report_sufficiency.py::_has_while_loop), refusal semantics untouched. Update skills/suggest-contacts/SKILL.md where it describes the terminal. Read the todo's "Operator ruling 2026-09-11" section first. Depends on nothing but shares suggest_contacts.py with the provenance-locator item. | 2026-09-11 | 20363788 | — | .planning/quick/260911-ao2-todo-2026-09-04-website-less-company-search-fallback-operato |
| 260911-ss4 | F1 (major, budget): enrich-before-ingest persists no step-2 match outcome, so every later SKILL fence that needs `classified` re-runs `preingest.match_batch` and re-POSTs the whole batch in propose mode (UAT 2026-09-11, run a254d1eda71246a2a964922cdf5c2bd2: six 4-row sends 12365/12366/12368/12370/12373/12374 + two 2-row sends 12375/12376). Persist the step-2 match outcome per run_id in the plugin durable state dir and make every later fence in operator-claude-plugin/skills/enrich-before-ingest/SKILL.md load it instead of re-matching; test against the recorded shape. See .planning/UAT-autonomous-batch-2026-09-09.md F1. | 2026-09-11 | d6f78c1e | — | .planning/quick/260911-ss4-f1-major-budget-enrich-before-ingest-persists-no-step-2-matc |
| 260911-ss5 | F9 (major, AFTER-01): the end-of-run report line "Matches the original batch's N row(s)" is scoped by SKILL.md line 1139 to len(unmatched_rows) and reads as false against a 4-row input; rename the scope line so it names this run's enrichment scope, and persist the matched-id handoff (confirmed_ids to enrich-records) per run_id as a durable file the report names. See .planning/UAT-autonomous-batch-2026-09-09.md F9. | 2026-09-11 | af7b8d59 | — | .planning/quick/260911-ss5-f9-major-after-01-the-end-of-run-report-line-matches-the-ori |
| 260911-ss6 | F11 (minor): operator-claude-plugin/skills/enrich-before-ingest/SKILL.md has no write_grant.close_grant step; add the batch-complete close (CLOSED_BATCH_COMPLETE) at end of run and CLOSED_SESSION_END guidance. See .planning/UAT-autonomous-batch-2026-09-09.md F11. | 2026-09-11 | b47d2296 | — | .planning/quick/260911-ss6-f11-minor-operator-claude-plugin-skills-enrich-before-ingest |
| 260911-ss7 | F10 (major, cost model): grant pricing and preview_enrichment price Lusha at 1 credit per contact; execution 12372 Lusha Enrich billing block shows creditsCharged 7 for one rich reveal (balance 3860 to 3853 confirmed) and 0 for NOT_FOUND. Re-measure the Lusha rate the pricing table uses and record the 7-credit observation in docs/LUSHA-V3-CONTRACT.md. Bump operator-claude-plugin to 0.46.0 with one CHANGELOG section naming F1/F9/F10/F11. See .planning/UAT-autonomous-batch-2026-09-09.md F10. | 2026-09-11 | 058ee899 | — | .planning/quick/260911-ss7-f10-major-cost-model-grant-pricing-and-preview-enrichment-pr |
| 260911-w6o | F2-1 (prerequisite): the held-queue entry stores the SOURCE row, so an enriched reveal is lost from disk (run a254d1eda71246a2a964922cdf5c2bd2: Jimmy Busteed's held entry has email "" while the waterfall returned jbusteed@australianturfclub.com.au, phone, mobile, LinkedIn). Persist the MERGED/enriched row into the held entry in operator-claude-plugin/skills/enrich-before-ingest/SKILL.md's persist step (held_queue.build_entry input), keep the forbidden-name guard on names only, and pin it with a test on the recorded run shape. Operator ruling 2026-09-11 (F2): no_match holds are faceted at read time; review-triage reads both queues in one table; create lands through the ingest lane under a review-lane grant; the batch renders the ready answer and never asks. | 2026-09-11 | 3dfbbb44 | — | .planning/quick/260911-w6o-f2-1-prerequisite-the-held-queue-entry-stores-the-source-row |
