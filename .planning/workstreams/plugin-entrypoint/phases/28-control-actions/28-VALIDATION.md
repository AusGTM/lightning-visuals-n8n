...
| 28-04-02 | 28-04 | 3 | CONTROL-03, CONTROL-05 | T-28-18 | per-job enable/disable is a scope decision, not a planner's call — five Schedule Triggers share one workflow | **manual (decision checkpoint)** | n/a — see Manual-Only Verifications | n/a | ⬜ pending |
...
| Per-job schedule enable/disable — widen the allowlist by one field, or refuse | CONTROL-03, CONTROL-05 | `LV Scheduled Maintenance (Cloud)` carries five Schedule Triggers, so workflow-level on/off cannot express it. CONTEXT says allowlist widening is a new requirement, not a planning decision | 28-04 Task 2, a `checkpoint:decision`. Choose `widen-by-one-field` or `refuse-per-job`; either way a REQUIREMENTS.md amendment follows |
