...
| 30-02-03 | 02 | 2 | REVIEW-03 | T-30-05 / T-30-06 | Unarmed request previews and never reaches the gate; injected keys change nothing | unit | `node --test tests/n8n/reviewDecisionEndpoint.test.mjs` | ❌ W0 | ⬜ pending |
...
| 30-05-01 | 05 | 5 | REVIEW-01, REVIEW-02 | T-30-20 / T-30-21 | Secret never rendered; policy lookup is display-only | unit | `.venv/bin/python -m pytest operator-claude-plugin/tests/test_review_queue.py operator-claude-plugin/tests/test_transport_guard.py -q` | ❌ W0 | ⬜ pending |
...
| 30-06-01 | 06 | 6 | REVIEW-03, REVIEW-04, REVIEW-05 | T-30-24 / T-30-25 / T-30-26 | Unarmed submit sends nothing; verified decided by property comparison, not status code | unit | `.venv/bin/python -m pytest operator-claude-plugin/tests/test_review_decision.py operator-claude-plugin/tests/test_transport_guard.py -q` | ❌ W0 | ⬜ pending |
...
- [ ] `operator-claude-plugin/tests/conftest.py` — programmable stub response payload plus the queue fixture (30-05 Task 1); the autouse network guard already exists and must stay intact
...
|----------|-------------|------------|-------------------|
| An armed review decision lands in HubSpot on exactly one allowlisted record, and the armed window is closed afterwards | REVIEW-02, REVIEW-03, REVIEW-04, REVIEW-05 | Agent tooling in this repo is blocked from arming writes, and no automated verification in this phase may perform a live HubSpot write | 30-07 Task 2's `checkpoint:human-verify`, steps 1-10: snapshot, armed deploy with a single-record allowlist, activate, unarmed preview, reject, approve, disarmed redeploy with read-back confirmation, compare snapshot |
