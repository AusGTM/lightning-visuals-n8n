...
- [x] **TIER-01**: `lv_icp_tier` no longer depends on a HubSpot property-change event to be
...
| TIER-01 | Phase 50 | **Complete** — Plan 05 (D-23) extended D-07's accepted-mismatch set from 4 to 5 known stuck records, absorbing the Coffs Harbour Racing Club residual (`14752488879`, `WINDOWS.md` id 14) this row previously left open; `50-TIER-PARITY-EVIDENCE.md`'s current verdict reads **PASS — zero defect rows; all 5 known stuck records read the exact expected mismatch**. `lv_icp_tier_derived` reproduces WF1's live ladder exactly against real records with no defect rows outstanding. Plan 05 additionally switched WF1 (`4625147345`) off live and verified by independent re-read (D-08) — the derived tier now runs with zero dependency on any HubSpot property-change event or workflow execution, which is the literal text of this requirement. **Not the same as full retirement:** archiving the old `lv_icp_tier` enum (D-06) is a separate, still-blocked action — see TIER-03 and `50-RETIREMENT-RECORD.md` — but TIER-01's own bar (a derived tier that no longer depends on the event) is met independent of that archive. |
...
| TIER-03 | Phase 50 | **Complete** — `50-DEPENDENTS-SWEEP.md`: scripted sweep (0 lists, 10 flows scanned, 5 findings all on WF1 itself, no external dependent) plus the operator-attested, dated manual check confirming saved views migrated off `lv_icp_tier` (2026-08-14). WF1 (`4625147345`) was switched off live and verified 2026-08-14, then the archive of `lv_icp_tier` was rejected live by HubSpot (`CANNOT_DELETE_PROPERTY_IN_USE` — WF1's actions still referenced the property even while disabled), a platform constraint not anticipated by this phase's research. **D-24 (operator, 2026-08-14) explicitly overrode D-08** (WF1's "not deleted" prohibition) to unblock this: WF1 was DELETED outright (`DELETE /automation/v4/flows/4625147345` → 204, independently re-read → 404), `lv_icp_tier` then archived cleanly on the first retry (`DELETE /crm/v3/properties/companies/lv_icp_tier` → 204, confirmed absent live and present under `?archived=true`), and `lv_icp_tier_derived` relabelled to "ICP Tier" (D-15's fallback, verified by re-read and a two-point D-22 poll on 3 known records). Rollback is now rebuild-from-JSON via `POST /automation/v4/flows` rather than a one-action re-enable — the proven manual-enrolment rollback mechanism no longer exists (see `docs/OPERATOR-TIER-ROLLBACK.md`'s amendment). Reports/dashboards remain explicitly UNCONFIRMED (no public API to enumerate them) — accepted, disclosed risk, not resolved. Full record in `50-RETIREMENT-RECORD.md`. |
