...
create them (operator authorised, classifier disabled). Any NEW property for this system
must be `lv_`-prefixed.
...
## The `lv_` naming instruction — noted, nothing to apply it to

No property was created, so the rule had no subject. Recorded for the next one.
...
**Renaming the existing 7 is a migration, not a create, and is NOT recommended:**
