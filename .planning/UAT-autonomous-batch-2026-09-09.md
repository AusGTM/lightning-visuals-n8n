...
- result: pass on the ask/pause/open, with one qualification from the verbose transcript: `watch.pre_spend_pause()` (a real `time.sleep`, D-68-05) ran INSIDE the same Bash-driven Python process that then called `open_grant` and dispatched — the pause is real in code but invisible to the operator, and the only interrupt available during it is killing the Bash call. No price-line/next-line timestamps exist because both lines came out of one tool call. Ceiling `ok` — 245 spent, 2255 remaining of 2500, 7 projected (run_audit: 247/2253 at report time). Grant scope: 0 record ids + 1 domain (`athertonturfclub.com.au`), allow-create on. Timestamps of the pause not captured (operator did not record them). Deviation: ALL THREE provider balances read `unconfirmed`, not only Apollo — see F3.
...
- result: NOT EXERCISED — no ingest send was made, so no ingest arming happened (`AwbBeShdPgV48eiY` `updatedAt` still 2026-09-10T14:11:04Z, Gate 12's disarm). Enrichment: `950HPb7a1GgSAIyZ` `updatedAt` 2026-09-11T10:03:16Z — one PUT, two seconds after 12372 stopped (10:03:14Z), matching run_audit's `disarm.outcome: disarmed` (saved 10:03:18Z) with every flag "false" and `TEST_RECORD_DOMAINS`/`TEST_RECORD_IDS` "". The verbose transcript settles the arm: `write_grant.authorize_send(grant, lane="enrichment", record_ids=[], record_domains=["athertonturfclub.com.au"])` returned `armed: True` ("bounded to this send's 0 record id(s) and 1 domain(s)"), and `n8n_arming.armed_window(...)` was entered around 10:02:20Z (after 12370, before 12372 at 10:03:02Z) and exited with `disarm_result` recorded. So the enrichment half of test 7 PASSES: one record-scoped window opened and closed around one send, `updatedAt` 10:03:16Z is its disarm PUT, and 12372 (`mode: propose`, no gate node ran) wrote nothing inside it.
...
- **F3 addendum:** `Lusha Usage`, `Apollo Usage` and `ZoomInfo Usage` nodes all RAN on 12372, so the backend read balances during the run; the plugin's report still says `not_reported_by_status_endpoint` for all three. Cause: `cost_guard` reads balances only through `backend_status.fetch_backend_status` (the `hubspot/backend-status` workflow, `Cj83mOgrIm59oxcX`), and that endpoint does not carry them — the enrichment workflow fetches balances the status workflow never exposes.
- **F9 (major, AFTER-01, by skill design): the end-of-run report is not the whole account.** "Row accounting 2/2 — matches the original batch's 2 row(s)" is false against the 4-row input — but it is what `skills/enrich-before-ingest/SKILL.md` lines 1125-1139 instruct: `original_row_count=len(unmatched_rows)`, a 2026-09-09 same-day correction that stopped a false MISMATCH on every batch with an ordinary matched row, because matched rows "never call `run_state.start_run` at all — they go to `enrich-records` via `confirmed_ids`, a different run entirely". Consequence: John Miller (3601) and Craig Sheppard (3501) — and batch 2's 7101 / 37400807974 — have NO durable record anywhere (no run_state, no report, no handoff file); the wording "original batch" is wrong; and every-row-back-exactly-once for the 4-row input is demonstrable only from the chat. Fix shape: keep the 2/2 scope but name it ("this run's enrichment scope"), and persist the matched-id handoff per run.
- **F11 (minor): the grant is never closed.** The SKILL has no `write_grant.close_grant` step (grep: 0 hits); `open_grant` writes nothing durable, so the grant lived in one Python process and died with it. "Batch-1 grant stays open in-conversation until session end" describes nothing that exists; `CLOSED_BATCH_COMPLETE`/`CLOSED_SESSION_END` were never recorded.
