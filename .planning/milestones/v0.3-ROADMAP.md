# Milestone v0.3: Company Enrichment & ICP Research

**Status:** ✅ SHIPPED 2026-07-29
**Phases:** 11 – 16.10 (16 phases, 10 of them INSERTED)
**Total Plans:** 22
**Timeline:** 2026-07-20 → 2026-07-29
**Closeout:** verified — every phase carries a `VERIFICATION.md` with `status: passed`; `audit-uat` reports `total_items: 0`.

> **Closeout note on tooling.** `gsd-tools query init.manager` projects phases 16.6, 16.8, 16.9 and
> 16.10 as `phase_complete: false / not_required`. This is a bookkeeping artifact, not a verification
> gap: 16.6 has no `SUMMARY.md` because its work landed through debug sessions and quick tasks rather
> than a plan, and the other three do not match the shape the projection expects. All four have
> `VERIFICATION.md` with `status: passed`. Same class as the known `state.update-progress` miscount
> across this repo's three concatenated milestones. The verification files are the source of truth.

---

## Requirements

All 13 v3 requirements complete. Full text and traceability archived alongside this file in
`v0.3-REQUIREMENTS.md`.

---

## Roadmap (verbatim from ROADMAP.md at close)
...
> Two rows below were `gaps_found` when their `VERIFICATION.md` was written and were closed by
> later commits; the verification files' `gaps:` blocks are stale, their `status: passed`
> frontmatter is current. `audit-uat` reports `total_items: 0` across all sixteen phases.
...
| 16.6. Companies Search Transport Fix (INSERTED) | 1/1 | **Complete** — criterion 7 verified live (execution 12, non-null Merge Company); criterion 1 closed at 6/6 search nodes executed live (`322574a`, `a076595`), after the VERIFICATION.md gap block was written | 2026-07-29 |
...
| 16.9. Create-Path Fix (BUG 13) + Company Writes (INSERTED) | 1/1 | **Complete** — `company:update` verified live, BUG 13 closed (`6d2565c`); SC-2 schema-coverage guard landed (`239b8af`) and SC-4 `company:create` met through the n8n node (`aa749f8`), both after the VERIFICATION.md gap block was written | 2026-07-29 |
...
**Inserted phases (10 of 16).** Milestone 3 was planned as five phases (11–15) and closed as
sixteen. Every insertion after 16.1 was driven by a defect that only a live run could expose —
the milestone's defining characteristic.

- 15.5 Tiered Candidate Adjudication — scoring must rank candidates, never decide them
- 16.1 Provider Selection, Credit Reporting & Schedule Safety
- 16.2 Contacts Research + Judge Mirror
- 16.3 Companies Stale-Timestamp Fix
- 16.4 Fetch-By-ObjectId
- 16.5 Deliberate Research/Escalation Enablement
- 16.6 Companies Search Transport Fix (BUG 10)
- 16.7 Write-Path Canary — the first HubSpot write in project history
- 16.8 Row-Carry Fix (BUG 12)
- 16.9 Create-Path Fix (BUG 13) + Company Writes
- 16.10 Write-Gate Coverage (BUG 15) + Live Escalation

**Key decisions:**

- **Companies is a sibling branch, not nested under contacts.** ICP fields are per-domain and
  expensive; nesting would re-pay for every contact at the same company.
- **Resolution is ordered deterministic → retrieval → judgement.** An LLM judging from parametric
  recall is least reliable exactly where the ICP lives — it knows Harvey Norman and confabulates on
  obscure ANZ clubs.
- **The pipeline writes ICP inputs only; HubSpot owns the derived outputs.** `lv_icp_fit_score`,
  `lv_icp_tier`, `lv_anti_icp_flag`, `lv_anti_icp_reason` and `lv_recommended_motion` are computed
  in HubSpot. Supersedes CLAUDE.md §29. `src/icp_scoring.py` still computes score and tier
  internally for routing and audit, but no longer reaches a PATCH.
- **`lv_produces_content` is tri-state.** A failed search is not evidence of absence — thin
  evidence must yield `null`, never `false`, because `false` fires a hard veto and thin-web-presence
  ANZ clubs are the ICP core.
- **`config/taxonomy.yaml` is the single hand-edited vocabulary.** Adding an org type or content
  type is a one-file edit; drift is a test failure, not a silent zero score.
- **Fail-closed everywhere.** A lookup failure downgrades `create` to `skip`; an unmapped credential
  now refuses deploy by node type; a write gate that cannot prove a record is allowlisted refuses
  the write.

**The milestone's central finding.** Measured against five live prospect accounts on 2026-07-20:
`lv_org_type` is resolvable from provider descriptions for 3 of 5 accounts, `lv_produces_content`
for **0 of 5** — no provider description mentions broadcast or streaming. Since
`lv_produces_content: false` fires a hard veto, retrieval was never an optimisation. It is the only
way to score the field at all.

**What was proven live.** The pipeline wrote to HubSpot for the first time (execution 15,
contact 201) and the non-clobber guarantee held under adversarial conditions: `seniority` filled
from null, while `mobilephone` kept its human value against a provider candidate that *cleared* the
85 confidence threshold and was refused by ownership class alone; `jobtitle` kept a wrong-but-human
value; `email` was refused below threshold. Exactly one record modified. Containment was proven by
demonstration, not assertion — with writes armed, an un-allowlisted company was refused with
`write_blocked` and no write node appeared in the run data. Sonnet escalation was exercised live
(execution 18): the judge adjudicated a real conflict and refused to promote the contested field.
The full review loop closed on a real record (execution 56): flag → decision JSON → approve →
apply → clear, four properties mutated and verified, canary deleted.

**Issues resolved (25 numbered bugs).** A representative slice:

- BUG 10 — n8n's HubSpot node with `resource: company, operation: search` emitted one item whose
  `json` was null, while the byte-identical raw request succeeded. Six nodes affected, none had ever
  run live. Replaced with credential-bound HTTP POSTs.
- BUG 11 — both update nodes shipped with an empty `updateFields` map referencing the patch nowhere.
  Caught **offline at plan time**; a canary against them would have written nothing and the phase
  criterion would have passed vacuously.
- BUG 12 — a `set` v3.4 node emitted only its assigned fields, destroying `merge`/`existingRecord`/
  `object_id`/`scored` en route to Decide Action. The contacts write path could not write to any
  record under any flags.
- BUG 17 — Lusha company enrichment had never worked. `onError: continueRegularOutput` put the 400
  in the item rather than the node, so every company run reported success while silently enriching
  from two providers instead of three.
- BUG 24 — `TEST_RECORD_DOMAINS` was silently inert on every company write lane, because `domain`
  was absent from the upstream search request.
- BUG 25 — Apply Review reconstructed its row and dropped `domain` before the gate, making BUG 24's
  fix inert.

**The recurring failure class.** Nearly every late bug was invisible offline and fail-closed live:
a provider error living in the item instead of the node, a field surviving the request but not the
node pipeline, a node emitting a subset of its input. Field *survival* through the whole pipeline
matters more than field presence in the upstream request. Contract tests that walk upstream to find
a field's real source (`_emitted_fields`, `_spreads_search_row`) were added to catch the shape
rather than the instance.

**Issues deferred to v0.4:**

- **BUG 23** — enrichment `contact:create` is structurally unreachable. Root-caused, not fixed.
  Requirements in `.planning/debug/bug-23-enrichment-contact-nomatch-chain-stop.md`.
- **ZoomInfo numeric industry.** ZoomInfo returns `industry: "71"` — a numeric code — which won the
  waterfall over Apollo's `"media production"` and normalized unchanged. Correctly routed to
  `needs_review` so nothing was clobbered, but a numeric code winning a text field is a
  normalization gap with no ticket.
- **Six `/gsd-verify-work` re-runs** carried from the original goal ledger.

**Technical debt incurred:**

- **Two latent copy-loop gaps.** `lv_sponsorship_reliant` (companies) and `persona_group`/
  `lv_persona_group` (contacts) are declared in policy and have HubSpot properties, but the
  production merge wrapper never copies either from its candidate source into the merge call. Both
  stay permanently empty until a one-line wrapper fix.
- **`lv_country_region_normalized` has no explicit `field_policy.yaml` entry** — falls through to
  the default `fill_blank_only`. Property created; the policy question flagged, not resolved.
- **`lv_icp_tier` options are `A,B,C,D` only**, but the scorer also emits `Unscored` and
  `Needs Review`. Writing those would fail today. Predates this milestone; not reached because the
  pipeline no longer writes tier.
- **`lv_org_type` is text, not an enumeration.** The text→enumeration change is a one-way door
  (HubSpot documents no API-level undo beyond restoring a pre-change export) and was deliberately
  **not performed** rather than gated as a disabled step.
- **The scoring precedence rule is a spec-by-example liability.** `src/icp_scoring.py`'s
  confidence-downgrade block once outranked an already-fired hard veto's tier label. Fixed in
  Python, but `icp_scoring.py` is the spec for the HubSpot-side formula that has not been authored
  yet — the formula's author inherits the bug unless the rule is carried across explicitly:
  **a hard veto's label wins over a confidence downgrade.**
- **`src/merge_policy.py:279-287`** still stamps cache-key `verified_at` on candidate selection
  rather than on promotion — the Python harness's own copy of a bug already fixed in both JS merge
  modules.
- **HubSpot-side score/tier formula is still the literal `1 + 1` placeholder.** Every company
  currently scores 2. Authoring the real calculation is downstream work, explicitly out of scope
  here, and this milestone's whole purpose was to make its inputs trustworthy first.
