...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 7 (MEDIUM-1): `preflight_before_send(..., lane=REVIEW_LANE)` over a backend where
      ONLY the review flag reads live returns the grant unchanged and no refusal, and the
      grant's state is still open — the batch window cannot trip over its own arm.
    - Test 8 (MEDIUM-1): the same call over a backend where a DISPATCH flag reads live still
      closes the grant and refuses, so the exclusion narrows the review flag and nothing
      else. Running Test 7 against the un-narrowed tuple is what fails, which is the property
      the pair exists to pin.
  </behavior>
...
**Then close the trap structurally rather than by documentation (cross-AI review, MEDIUM-1, 2026-09-01).** Task 1 widens the local `WRITE_ENABLING_FLAGS` to include the review flag, which gives `preflight_before_send` a self-trip: called on the review lane while a batch window is open, it would read the window's OWN arm as "writes still live", close the grant, disarm mid-batch and refuse the send. The review found no production caller today, so this is latent — and a latent trap fixed only by prose is a trap that fires the first time someone wires the pre-flight into a triage loop, because the caller reads `preflight_before_send`'s docstring and not `authorize_review_batch`'s. Fix it in the function itself: `preflight_before_send` already takes `lane`, so evaluate liveness over the dispatch flags alone when the lane is `REVIEW_LANE`, and over the full widened tuple for every other lane. Derive the narrowed set from the same tuple (drop the review flag from it) rather than restating a second literal list, so the two can never drift.

Note carefully what this does NOT weaken: a live DISPATCH flag found on the review workflow is still a genuine anomaly and still closes the grant, because only the review flag is excluded and only for the review lane. What it deliberately gives up is the pre-flight's ability to notice a review flag left live between sittings — which is Guardrail A's job at the next grant open, where refusing costs nothing, and which the batch window's own guaranteed disarm covers within a sitting. Record all of that in `preflight_before_send`'s OWN docstring — the function a future caller actually reads — with a dated note naming Phase 60 and the widened tuple as the reason the exclusion exists. Cross-reference it in one sentence from `authorize_review_batch`'s docstring so a reader meeting the batch window first is sent to the guard, but state the mechanism as a guard and never as a prohibition: with the exclusion in place, calling the pre-flight on the review lane mid-batch is harmless rather than forbidden.

Add the two guard tests: a review-lane pre-flight over a backend where ONLY the review flag reads live returns the grant unchanged with no refusal and does NOT close it; a review-lane pre-flight over a backend where a dispatch flag reads live still closes the grant and refuses, exactly as it does for any other lane.
...
Add the six tests above to `test_write_grant_guardrails.py`, in a new section headed with a dated comment naming D-60-06 and stating that no existing test in this repo exercised a multi-decision single window for any lane, so this coverage is genuinely new rather than a copy. Drive them through the existing `stub_module_transport_factory` recorder and assert on the recorded call sequence, not only on returned dicts — the "arm once, disarm once" property is only visible in the call log.
  ...
  <acceptance_criteria>
    ...
    - Source assertion: `grep -c 'preflight_before_send' operator-claude-plugin/scripts/write_grant.py` is at least 2 — its own definition plus the cross-reference from `authorize_review_batch`'s docstring, so a reader who meets either function first is pointed at the guard.
    - Behavior assertion (MEDIUM-1): the review-lane pre-flight test asserts the returned grant's state is still open and that `_close_with_disarm` was not reached (no disarm PUT in the recorded call log), not merely that the refusal was `None`.
    - Behavior assertion (MEDIUM-1): the dispatch-flag-live companion test asserts the grant IS closed, proving the exclusion is scoped to one flag on one lane.
...
| T-60-08 | Denial of Service | `preflight_before_send` on the review lane mid-batch | medium | mitigate | Raised as documentation-only by cross-AI review (MEDIUM-1) and closed structurally instead: `preflight_before_send` evaluates liveness over the dispatch flags alone when `lane == REVIEW_LANE`, so the batch window cannot read its own arm as live, close the grant and disarm mid-batch. A live dispatch flag on the review workflow still closes. The prohibition now lives in `preflight_before_send`'s own docstring as an explanation of the guard, not as a rule a future caller must remember. Tests 7 and 8 pin both halves. |
...
- `write_grant.preflight_before_send` — review-lane liveness narrowed to the dispatch flags, documented in its own docstring (MEDIUM-1)
