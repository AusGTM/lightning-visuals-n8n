...
<task type="auto" tdd="true">
  ...
  <behavior>
    ...
    - Test 8 (LOW-4): an item with no `object_type` key classifies to the company object
      type, and the assertion's docstring records that this fallback is unreachable from
      `submit_decision`, whose own required argument always supplies the key.
  </behavior>
...
Add `classify_review_item(item)` beside `classify_item`, pure and no I/O, raising `WrittenRecordsError` on a non-dict for the same fail-loud reason. It reads `object_type`, `record_id`/`hs_object_id`, `decision` and `outcome` off the item and emits EXACTLY `classify_item`'s seven keys: `object_type`, `action` as `review_approve` or `review_reject` derived from the decision word (anything else is `review_unknown`), `hs_object_id`, `outcome` from the mapping with `FAILED` as the total fallback, and `reason`, `row_id`, `association` all `None`.

**The `object_type` fallback, resolved from the endpoint's real response shape (cross-AI review, LOW-4, 2026-09-01).** The review endpoint's success return carries exactly `available`, `reason`, `outcome`, `message`, `would_write`, `verified_properties` and `verified` — no object type and no record id at all. The reviewer's either/or ("dead code if the response carries it, wrong on the common path if it does not") resolves to a third case: the classifier's input is not the response. It is an item Task 2 builds locally from `submit_decision`'s own required `object_type` argument, so the key is always present on the only call site that exists and the fallback is unreachable by construction — not by the response shape. Record that in the function's docstring rather than leaving a reader to re-derive it. Set the fallback to `"companies"`, NOT to the value `classify_item` falls back to: an analogy to a different classifier is not a reason, and if a future hand-built item ever omits the key, this system's review queue is company-centric, so the company object type is the honest guess and the contact one would silently mislabel the common record. Say both halves in the docstring — that the fallback should never fire, and what it means if it ever does. Document why `reason` is deliberately `None` and must stay so: the operator's own words already live on the record itself in `lv_enrichment_review_reason`, so the artifact loses nothing by omitting them — and free operator prose containing `arm`, `grant` or `permission` would trip `_looks_forbidden`, which is a substring check, and raise on a bookkeeping write that must never be able to raise. Run the same `_looks_forbidden` sweep over the finished entry that `classify_item` runs, so the Phase 23 D-11 guarantee holds identically on this path.
...
Add the seven tests above to `test_written_records.py`, in a section headed with a dated comment naming D-60-08 and stating why the review response could not be fed to `classify_item` unmodified: it carries no `action` key at all, so `outcome_for_action(None, ...)` would resolve every single review decision through the `FAILED` fallback — an approve that landed would be filed as a failure. Keep the file's existing discipline of redirecting `written_records_path` to a `tmp_path` through monkeypatch; never touch the operator's real durable directory.
  ...
  <acceptance_criteria>
    ...
    - Behavior assertion (LOW-4): `classify_review_item` on an item with no object-type key returns the company object type, and its docstring states the endpoint response carries no object type, that the item is built locally by `submit_decision`, and that the fallback is therefore unreachable on the only call site.
...
Add the six tests above to `test_review_decision.py`, monkeypatching `written_records.written_records_path` to a `tmp_path` file exactly as `test_written_records.py` does, so the operator's real durable directory is never touched.
  ...
  <acceptance_criteria>
    ...
    - Behavior assertion (LOW-4): the entry written for a company decision carries the company object type and the entry for a contact decision carries the contact one — `submit_decision` passes its own argument through and the classifier's fallback never decides.
