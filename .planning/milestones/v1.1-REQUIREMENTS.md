...
(updated 2026-09-02). Phases 53–63: 53, 54, 57, 58, 59 and **61** complete; 55 and 56 absorbed
into 61 (D-61-08); **60 is open**; **62 has executed** (verification `human_needed` 13/13, UAT
partial — closes SUGGEST-01/-02/-04/-05, amends SUGGEST-03) but is not complete; **63 is
numbered (2026-09-02) but not yet planned**.
**Prior milestone:** v1.0 Direct Backfill & Scoring Coverage — Phase 51 complete, **Phase 52
deferred INDEFINITELY by the operator (2026-08-25)** — not merely in favour of this milestone.
Its requirements are the root `.planning/REQUIREMENTS.md`; nothing in this milestone is tracked
there.

**The first live unattended, credit-spending batch has NOT run.** Phase 57 landed 2026-09-01
(ceilings, refusal-before-start, post-run proof), so it is no longer the pending gate; Phase 61's
backend remains deployed and disarmed-proven only. RUN-05 and AFTER-03 are now ticked; **AFTER-01
remains PARTIAL** (57-05), which is the run requirement still standing between here and that
first live run.
...
- [x] **SUGGEST-01**: After companies are ingested or enriched with no contacts named, the
  system **suggests contacts worth enriching** rather than stopping. An enriched company with
  nobody at it is not a lead.
  > Closed by Phase 62 Plan 05: `skills/suggest-contacts/SKILL.md` is auto-offered right
  > after `enrich-records`' end-of-run report on a companies batch (D-62-15), naming how
  > many of that batch's companies came back with nobody at them.
- [x] **SUGGEST-02**: For a bulk company list the suggestion is **categorical, not per-record**
...
- [ ] **SUGGEST-03**: The role vocabulary is **derived from this portal's own contact records**,
  ...
  the existing pattern for inventorying a property's live values. **When the portal carries too
  few distinct `jobtitle` values to evidence a vocabulary, a generic role list MAY be served
  instead, PROVIDED it is labelled un-evidenced wherever it is shown** — the operator must be
  able to tell a portal-derived role from an invented one at a glance.
  > **Amended by D-62-07** (operator, 2026-09-02): the original text forbade a generic B2B list
  > with no exception. Phase 62 **AMENDS** this requirement rather than closing it as
  > originally written — the disclosed-fallback exception above is the amendment.
- [x] **SUGGEST-04**: Suggested contacts are **proposed, never auto-created**. They are people
  ...
  write.
- [x] **SUGGEST-05**: The cost of a suggestion round is shown before it is spent. Provider
  ...
  mid-run.
  > Closed by Phase 62 (re-scoped, D-62-01 rev 3): discovery is sitemap-ladder-sourced in
  > the plugin, not a provider people-search call. `write_grant.plan_grant()`'s folded
  > `suggestion_companies`/`suggestion_cap` prices the round's worst-case ceiling into the
  > SAME opening grant envelope the operator already sees before any spend (D-62-11).
