...
**Defined:** 2026-08-25. **Status:** DEFINED, NOT STARTED (phases unplanned).
**Prior milestone:** v1.0 Direct Backfill & Scoring Coverage (in flight, Phase 51 complete).
...
- [ ] **GRANT-01**: An operator can open a **session grant** in one exchange, stating what it
...
- [ ] **RUN-01**: A batch runs ingest → enrichment → HubSpot write end to end with no operator
  input between chunks.
- [ ] **RUN-02**: Chunk failures do not abandon the batch; failed records are collected and
  re-sendable as one well-formed request (today's `failed_batch`, carried through).
- [ ] **RUN-03**: Throughput is designed against the measured bounds, not assumed away — the
...
- [ ] **RUN-04**: A run reports progress the operator can read while it runs — records done,
...
- [ ] **AFTER-02**: Held and failed rows land in a queue that survives the session.
- [ ] **AFTER-03**: The report distinguishes "written" from "would have been written" — a
...
- [ ] **INPUT-01**: A company can be named by **anything the operator holds**: a screenshot of
...
- [ ] **INPUT-03**: A researched domain is **confirmed before it is written**, not accepted
  silently. Getting this wrong creates a company under the wrong domain, which is the dedupe
  anchor for everything after it.
- [ ] **INPUT-04**: A refusal is a last resort and must always name what would make it work.
...
- [ ] **VOCAB-05**: **The arming phrase dies.** An operator answers the question they were
...
Roadmap: `.planning/milestones/v1.1-ROADMAP.md` (phases 53–57).
