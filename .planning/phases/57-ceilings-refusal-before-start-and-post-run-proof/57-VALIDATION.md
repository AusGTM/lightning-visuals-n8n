...
> whole table. (1) Task numbers shifted: 57-01 and 57-02 each gained a leading
> `checkpoint:decision`, and 57-05 gained one at the end. (2) **Markdown identifier greps are no
...
|---|---|---|---|
| 57-01 · T1 (checkpoint) | none — blocking operator decision | Preceded by a live READ-ONLY `n8n_read.executions_in_window` measurement against the deployed instance, whose four values (`count_in_window`, `observed_span_hours`, `covers_full_window`, `truncated_by_page_cap`) are quoted in the checkpoint and recorded in the summary. This is the evidence for whether RUN-05's refusal can fire on this account at all | ⬜ |
| 57-01 · T2 (tracer) | `.venv/bin/python -m pytest operator-claude-plugin/tests/test_write_grant.py operator-claude-plugin/tests/test_chunking.py operator-claude-plugin/tests/test_n8n_read.py -q` | `write_grant.plan_grant()` for the refusal; a REAL 3-chunk `chunking.dispatch_plan()` with `stub_module_transport_factory` for the PRE-SEND stop (assert the transport is NOT called for the breaching chunk, and `len(results) == 2`); the resulting `DispatchOutcome` passed to `write_grant.record_dispatch_outcome` for the close — never a hand-built outcome. Plus `n8n_read.executions_in_window` itself for `listing_exhausted` / `max_pages` | ⬜ |
...
| 57-04 · T2 | `.venv/bin/python -m pytest operator-claude-plugin/tests/test_cost_guard.py -q` | `cost_guard.compare` and `write_grant._headroom` over the new fixture; the Apollo assertion pins `http_403` (`conftest.py:541`), NOT `unrecognized_response_shape` | ⬜ |
...
- [ ] ZoomInfo-specific `provider_error` response fixture — `conftest.py`'s existing `_balance()` / `backend_status_*` fixtures cover Apollo's **`http_403`** (`conftest.py:541`, NOT `unrecognized_response_shape` — the research and the first plan set were both wrong on this) but no ZoomInfo shape. Add only if G-4's fix needs a regression test.
...
- [ ] The live READ-ONLY month-to-date sampling measurement (57-01 T1's precondition). It is not a test, but it is the evidence that decides whether RUN-05's preflight refusal is reachable at all on this account, and it must be taken before the tracer is implemented.
