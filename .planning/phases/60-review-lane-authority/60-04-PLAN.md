...
<task type="auto">
  ...
  <precondition>The repo venv can import the builder — `.venv/bin/python -c "import sys; sys.path.insert(0, 'scripts'); import build_cloud_workflows"` exits 0. The builder has import-time side effects and a sibling-module import (`gen_taxonomy_js`) that system python does not satisfy; halt and report rather than regenerating with the wrong interpreter. Second precondition (LOW-6): running the builder BEFORE any edit leaves `git status --porcelain n8n/` empty. A non-empty baseline is pre-existing generator drift that predates this phase — halt and report it as its own finding rather than editing over it, since after the edit it cannot be told apart from this task's own diff.</precondition>

  <action>
**Step 0, before any edit: establish a clean baseline (cross-AI review, LOW-6, 2026-09-01).** Run `scripts/build_cloud_workflows.py` FIRST, with nothing changed, and read `git status --porcelain n8n/`. The builder rewrites every workflow file, so committed JSON that has drifted from its generator for any unrelated reason — routine in a long-lived repo — would otherwise surface only after the edit and halt this task for a problem it did not cause and cannot cleanly separate from its own diff. If that baseline status is non-empty, STOP: do not edit, do not commit, do not stage the drift. Report it in the summary as its own finding, naming each drifted file and the fact that it predates this phase, and let the operator decide whether it is fixed here or in its own change. If the baseline is clean, proceed — and every later status check in this task is now unambiguously this task's own diff.

Then, in `n8n/code/reviewDecision.js`'s `writeAllowed === false` branch, replace the trailing clause of the `not_allowlisted` message — the one asserting that only an administrator can put records on the allowlist, and only while deploying — with wording that is true after this phase: the allowlist is set either by a deploy or, now, dynamically for the duration of one authorized window, and the operator's route is to open a write grant covering this record rather than to find an admin. Keep the first half of the message byte-identical (that this record is not on the backend's allowlist, that nothing was sent to HubSpot, and that the record is unchanged) — it is still exactly true and is what the outcome means. Keep the outcome literal `not_allowlisted` untouched; `test_review_outcome_parity.py` pins it against this file and the committed JSON as text. Add a short dated comment above the message naming Phase 60 and D-60-05, recording that this string was accurate before a grant could arm the allowlist and is being corrected rather than rewritten, so a reader does not have to date it from `git blame`.

Then regenerate the workflow JSON by running the builder again — never by editing `n8n/wf_review_decision_cloud.json`. Because step 0 already proved the baseline clean, the resulting `git status` is this task's diff and nothing else: only the JS source and the review workflow may differ. Any other workflow showing a diff after a clean baseline means the edit reached further than intended — STOP and report rather than committing it alongside a message-text change.
  </action>
  ...
  <acceptance_criteria>
    ...
    - CLI output (LOW-6, step 0): a builder run BEFORE any edit leaves `git status --porcelain n8n/` empty. A non-empty baseline is pre-existing generator drift, is reported as its own finding, and stops this task before it edits.
    - CLI output: after the edit and the second builder run, `git status --porcelain n8n/` lists exactly the two files named above.
...
Rewrite step 6. The per-record ritual is UNCHANGED and must be said to be unchanged: read the exact write back, get an explicit yes for this record, and that yes is `review_armed=True` for that one submit and nothing else — VOCAB-05 still holds, an operator saying yes must never have to produce the system's wording. What changes is only the authority underneath it: `submit_decision` now takes `grant=` and `run_id=`, and refuses with reason `grant_not_authorized` when no open grant covers this record. Delete the paragraph telling the operator that an administrator must set an environment variable and that this skill cannot set it — that refusal no longer exists; put in its place the grant refusal's handling, which is genuinely actionable: relay the message, and offer to open a grant covering this record rather than sending them to an admin. Say one true sentence about what a reject without a grant actually achieves (cross-AI review, MEDIUM-3, 2026-09-01): the client sends it, but the deployed gate checks its record allowlist before it looks at the decision word, so a reject on a record no open grant covers comes back `not_allowlisted` and the record stays flagged. The skill must not promise the operator that rejecting always clears the queue entry — it should relay the outcome and offer the same remedy an approve gets, which is opening a grant covering the record. Keep the sentence that a yes here does not arm the contact-upload or enrichment lanes and vice versa — the per-record consent is still lane-specific even though one grant now spans three lanes; the grant is the authority, the yes is still the act.
