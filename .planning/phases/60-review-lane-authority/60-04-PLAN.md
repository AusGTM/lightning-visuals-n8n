...
Then regenerate the workflow JSON by running the builder again — never by editing `n8n/wf_review_decision_cloud.json`. Because step 0 already proved the baseline clean, the resulting `git status` is this task's diff and nothing else: only the JS source and the review workflow may differ. Any other workflow showing a diff after a clean baseline means the edit reached further than intended — STOP and report rather than committing it alongside a message-text change.
  ...
  <verify>
    <automated>.venv/bin/python scripts/build_cloud_workflows.py &amp;&amp; git status --porcelain n8n/ | wc -l</automated>
    <fails_when>non-zero exit, a traceback instead of one wrote-line per workflow, or a count above `0` — a non-zero baseline is pre-existing generator drift and this task stops there rather than editing over it (LOW-6)</fails_when>
