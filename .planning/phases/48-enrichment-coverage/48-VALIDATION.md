...
| **Quick run command** | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -x` · `node --test tests/n8n/researchErrorGateFlow.test.mjs` |
...
Task IDs bound by the planner 2026-08-12 against the committed 6-plan layout. Requirement/behavior rows are fixed. The driver module is `scripts/enrich_coverage_companies.py` and its test file is `tests/test_enrich_coverage_companies.py`.

| Task ID | Plan | Wave | Requirement | Threat Ref | Secure Behavior | Test Type | Automated Command | File Exists | Status |
|---------|------|------|-------------|------------|-----------------|-----------|-------------------|-------------|--------|
| 48-01-T1 | 01 | 1 | COVER-01 | T-48-01 | Tracer: one record travels the whole spine (live-search seam, captured evidence, literal decision, enum validation, estimate, budget gate, dry-run payload, unarmed-POST refusal) with zero network calls | unit, offline | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k tracer -x` | ❌ W0 | ⬜ pending |
| 48-01-T2 | 01 | 1 | COVER-01 | — | Each of the 4 already-researched records maps to the exact CONTEXT.md enum value (Jam TV→`broadcaster`, Waikato→`individual_club_team`, The Rumble→`content_producer`, Editix→`unknown`) | unit | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k mapping -x` | ❌ W0 | ⬜ pending |
| 48-01-T2 | 01 | 1 | COVER-01 | — | A record written `unknown` also carries a non-empty `lv_enrichment_review_reason`; `coverage_state()` proves it distinct from blank `lv_org_type` | unit | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k marker -x` | ❌ W0 | ⬜ pending |
| 48-01-T3 | 01 | 1 | COVER-01 | — | The live population is re-derived with the exact HAS_PROPERTY/NOT_HAS_PROPERTY filter pair, and `reconcile_population()` never narrows the run on drift | unit + live read | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k "population or reconcile" -x` | ❌ W0 | ⬜ pending |
| 48-02-T3 | 02 | 2 | COVER-01 (D-04) | T-48-03 (error payload consumed as data) | `IF Research Errored` routes a genuine `{error:...}` item to the failure branch, a healthy `{content:[...]}` item down the normal lane, and a degenerate `{}` to the failure branch — driven through the node's REAL emitted expression | node:test, offline, against committed `n8n/wf_enrichment_cloud.json` | `node --test tests/n8n/researchErrorGateFlow.test.mjs` | ❌ W0 | ⬜ pending |
| 48-02-T3 | 02 | 2 | COVER-01 (D-09 no-regression) | — | The recompute lane still routes correctly after D-04's node insertion — no node rename/renumber collision | node:test, offline | `node --test tests/n8n/companyRecomputeLaneFlow.test.mjs` | ✅ exists | ⬜ pending |
| 48-03-T1 | 03 | 3 | COVER-02 | T-48-02 (silent mid-run truncation) | `estimate_phase48_cost()` returns the Phase-48-shaped dict, and `refuse_if_over_budget()` **raises** on a synthetic over-budget estimate rather than truncating; within budget it returns the id list identical in length and order | unit, offline | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k budget -x` | ❌ W0 | ⬜ pending |
| 48-03-T3 | 03 | 3 | COVER-01 | T-48-01 (out-of-vocabulary enum write) | Racing NSW research output is constrained to the 9 live `VALID_ORG_TYPES` at the prompt; an out-of-vocab OR unevidenced value routes to the D-03 marker and never reaches a PATCH | unit, offline, synthetic response | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k racing_nsw -x` | ❌ W0 | ⬜ pending |
| 48-05-T1 | 05 | 5 | COVER-02 | T-48-04 (empty allowlist reports armed) | `assert_allowlist_exact()` raises on an empty allowlist, raises on a superset, passes only on the exact 5-id set | unit, offline | `.venv/bin/python -m pytest tests/test_enrich_coverage_companies.py -k "allowlist or window" -x` | ❌ W0 | ⬜ pending |
...
- [ ] `tests/test_enrich_coverage_companies.py` — new Phase 48 driver unit tests (tracer slice, mapping correctness, `unknown`+reason marker semantics, budget refusal, allowlist guard). Created in plan **48-01 T1**. Follow `tests/test_veto_remediation_report.py`'s pattern of asserting a classifier function's output against named fixture ids.
...
| `IF Research Errored` is present in the RUNNING instance | COVER-01 (D-04) | A stored read-back proves only that storage changed; the running instance may still hold old in-memory code (Trap #3) | Bound to plan **48-04 T3**: one disarmed recompute POST, then read that execution's OWN embedded workflow node list. Judge by node-level `runData`, never by `executionStatus` — n8n `status: "success"` lies (Trap #1) |
| ~~`IF Research Errored` fires on a real erroring Anthropic call~~ — **UNACHIEVABLE THIS PHASE, recorded not dropped** | COVER-01 (D-04) | No execution in Phase 48 traverses the research branch: the armed window is recompute-only (which bypasses providers/research/judge/merge) and Racing NSW's research is a standalone Python call. An Anthropic 400 cannot be induced on demand | Not attempted. The bar this phase meets is structural presence in the running instance (48-04 T3) plus the offline expression test against a genuine error payload, a healthy payload and a degenerate payload (48-02 T3). `48-RUN-REPORT.md` must state this limitation explicitly rather than imply the firing was proven |
