...
def is_uat_contact(props: dict) -> bool:
    ...
    slug = li.rsplit("/", 1)[-1]
    return any(local.startswith(m + ".") or slug.startswith(m + "-") for m in MARKERS)
...
def build_plan(domains: list[str], snapshot: dict, since: datetime | None, extra_company_ids=()) -> dict:
    ...
    for flt in (
                [{"propertyName": prop, "operator": "CONTAINS_TOKEN", "value": m + "*"}]
                for m in MARKERS for prop in ("email", "lv_linkedin_url")):
