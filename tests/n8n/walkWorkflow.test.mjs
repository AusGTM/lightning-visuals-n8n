...
test("NF-BL-01: a combine/combineByPosition Merge fired with an unfilled input ANNIHILATES " +
  ...
  assert.deepEqual(trace.stalled, [
    ...
    { node: "M", reason: "merge_dropped_rows", run: 0, itemCounts: { 0: 2 }, outputCount: 0 },
  ]);
...
test("NF-MJ-01 (KNOWN-UNOBSERVED, pinned): two grouped producers OVERLAPPING on an input of a " +
  ...
  assert.deepEqual(runData.M, [[{ id: "q" }, { id: "q" }]],
    "NF3-NT-04: the CHOICE pinned — Q's row twice, P's row nowhere; a per-input-queue model gives (p,p,q)");
  const undrained = trace.stalled.filter((s) => s.reason === "merge_pending_runs_undrained");
...
test("NF3-MJ-01 (MN-07 earliness): a chooseBranch Merge with BOTH inputs filled is refused " +
  "BEFORE it can drive a downstream node — the HTTP stub records zero calls", () => {
  const calls = [];
  const graph = wf(
    [
      triggerNode("Trigger"),
      ...
      { name: "Sink", type: "n8n-nodes-base.httpRequest", parameters: {} },
    ],
    {
      Trigger: { main: [[edge("A")]] },
      A: { main: [[edge("M", 0), edge("M", 1)]] },
      M: { main: [[edge("Sink")]] },
    }
  );
  assert.throws(
    () => walkWorkflow(graph, {
      triggerNode: "Trigger", triggerItems: [{}],
      httpStubs: { Sink: (items) => { calls.push(items.length); return []; } },
    }),
    /chooseBranch/
  );
  assert.deepEqual(calls, [], "without the creation-site refusal the Merge fires as append and Sink runs once");
});
