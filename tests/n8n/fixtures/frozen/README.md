...
| `exec_12434.runData.json` | `12434` (Phase 73 Plan 01, D-73-13, F-B5) — FULL runData, frozen live via `scripts/freeze_execution_rundata.py` from run `6891d018e84f4d869eb8080292dac6c5` (Attempt 2 Stage B, executions `12432`-`12449`). Two companies dispatched in this execution, both `create` (Racing Victoria, Perth Racing) plus one whole-request `research_failed` marker — this execution carries no `enrich` row. |
| `exec_12449.runData.json` | `12449` (Phase 73 Plan 01, D-73-13, F-B5) — FULL runData, same run. The LAST execution of the 18: the Illawarra name-only `skip` row, one `create` (Country Racing Collective, domain `gmail.com`), and one `research_failed` marker. |
| `run_6891d018-decide-and-response.excerpt.json` | ALL 18 executions of run `6891d018e84f4d869eb8080292dac6c5` (`12432`-`12449`), EXCERPTED to two nodes only (`Decide Company Action`, `Build Response`) per execution. Added beside the two named fixtures above because neither alone contains an `enrich` (update) row — this run's 24 enrich outcomes are spread across the other 16 executions — and `test_run_report_enrich_account.py` needs the whole run to prove D-73-11/D-73-12's reconciliation. Documented departure from "exactly two" fixtures (deviation, see `73-01-SUMMARY.md`). |

The full runData account of each is in
`.planning/phases/70-one-merge-one-result-channel-n8n-runtime-truth/70-UAT.md`.
`73-01-SUMMARY.md` (Phase 73) has the account for the three rows above.
